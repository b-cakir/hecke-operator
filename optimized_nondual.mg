...
Optimized Version (only for non-dual vector space)
...
matrix multiplication. This version realizes it through a ring homomorphism on
the polynomial ring K[X, Y], since we consider harmonic cocycles with values
in the vector space of homogeneous polynomials in two variables of a certain
degree. Harmonic cocycles with values in the dual space are not realized in
this version. The program optimized.mg contains both cases, but is not as fast
as this version in the non-dual case.

Additionally, the last function "operator" saves computed graphs in a list
...
function preparation(R, N, type, irr, r, Graph, Reps)
	F := BaseRing(R);
	p := Characteristic(F);
	K := FieldOfFractions(R);
	T := Name(K, 1);
	...
	if irr eq false then
		BasisFunctions := MonomialsOfDegree(Pol, r);
	elif irr eq true then
		BasisFunctions := [];
		
		for i := 0 to r do
			if Binomial(r, i) mod p ne 0 then
				Include(~BasisFunctions, X^i * Y^(r - i));
			end if;
		end for;
	end if;
	
	return BasisEdges, BasisFunctions, #BasisEdges * #BasisFunctions;
end function;
...
function cocycle(R, N, type, Graph, Reps, irr, r, twist, coeff, v, w)
	F := BaseRing(R);
	K := FieldOfFractions(R);
	T := Name(K, 1);
	...
	Pol<X, Y> := PolynomialRing(K, 2);
	BasisEdges, BasisFunctions, dimension := preparation(R, N, type, irr, r, Graph, Reps);
	n := Degree(N);
	dim := #BasisFunctions;
	...
	if Minimum(k_v, k_w) gt n + dim then
		...
		if #stab(R, N, type, gamma_rep, 0) eq 1 then
			...
			if counter eq 1 then
				...
				for i := 1 to dim do
					value := value + Pol ! coeff[(index - 1) * dim + i] * Pol ! BasisFunctions[i];
				end for;
				
	            emb := hom < Pol -> Pol | (1 / Determinant(gamma)) * (gamma[2][2] * X - gamma[1][2] * Y), 
	                          (1 / Determinant(gamma)) * ((-1) * gamma[2][1] * X + gamma[1][1] * Y)>;
	            
	            if twist ge 0 then
	            	value := Pol ! (Determinant(gamma)^(twist) * emb(value));
				elif twist lt 0 then
					value := Pol ! ((1/(Determinant(gamma)^(Abs(twist)))) * emb(value));
				end if;
				...
				if gamma_rep ne B_gamma_rep then
					value := value - Pol ! cocycle(R, N, type, Graph, Reps, irr, r, twist, coeff, v_norm, B_norm);
				end if;
				...
				for z in F do
					...
					if gamma_rep ne A_gamma_rep then
						value := value - Pol ! cocycle(R, N, type, Graph, Reps, irr, r, twist, coeff, v_norm, A_norm);
					end if;
				end for;
			end if;
		else
			...
			if #stab(R, N, type, B_gamma_rep, 0) eq 1 then
				temp2 := Pol ! cocycle(R, N, type, Graph, Reps, irr, r, twist, coeff, v_norm, B_norm);
				
				sta := stab(R, N, type, gamma * gamma_rep, 0);
				
				for g in sta do
					emb := hom < Pol -> Pol | (1 / Determinant(g)) * (g[2][2] * X - g[1][2] * Y), 
	                          (1 / Determinant(g)) * ((-1) * g[2][1] * X + g[1][1] * Y)>;
	                
	                if twist ge 0 then
	            		value := value - Pol ! (Determinant(g)^(twist) * emb(temp2));
					elif twist lt 0 then
						value := value - Pol ! ((1/(Determinant(g)^(Abs(twist)))) * emb(temp2));
					end if;
				end for;
			else
				for z in F do
					...
					if #stab(R, N, type, A_gamma_rep, 0) eq 1 then
						temp2 := Pol ! cocycle(R, N, type, Graph, Reps, irr, r, twist, coeff, v_norm, A_norm);
		...
		value := Pol ! (-1) * cocycle(R, N, type, Graph, Reps, irr, r, twist, coeff, w, v);
		...
		if #stab(R, N, type, gamma_rep, k_v) eq 1 then
			...
			if k_B eq k_v - 1 then
				value := value + Pol ! cocycle(R, N, type, Graph, Reps, irr, r, twist, coeff, B_norm, v_norm);
			end if;
			...
			for z in F do
				...
				if k_A eq k_v - 1 then
					value := value + Pol ! cocycle(R, N, type, Graph, Reps, irr, r, twist, coeff, A_norm, v_norm);
				end if;
			end for;
		else
			stable := false;
			...
			temp2 := Pol ! cocycle(R, N, type, Graph, Reps, irr, r, twist, coeff, i_norm, t_norm);
...
function space(R, N, irr, r, twist, type, Graph, Reps)
	K := FieldOfFractions(R);
	Pol<X, Y> := PolynomialRing(K, 2);
	T := Name(K, 1);
	...
	if type eq 2 or type eq 3 then
		BasisEdges, BasisFunctions, dimension := preparation(R, N, 1, irr, r, Graph, Reps);
	elif type eq 4 then
		BasisEdges, BasisFunctions, dimension := preparation(R, N, 0, irr, r, Graph, Reps);
	end if;
	...
	for g in SOR do
		...
		for i := 1 to dimension do
			...
			for j := 1 to m do
				...
				if type eq 2 or type eq 3 then
					value := cocycle(R, N, 1, Graph, Reps, irr, r, twist, coeff, g * vertex, g * vertex * Matrix(K, 2, 2, [1, 0, 0, 1/T]));
				elif type eq 4 then
					value := cocycle(R, N, 0, Graph, Reps, irr, r, twist, coeff, g * vertex, g * vertex * Matrix(K, 2, 2, [1, 0, 0, 1/T]));
				end if;
				
				emb := hom < Pol -> Pol | (1 / Determinant(g^(-1))) * ((g^(-1))[2][2] * X - (g^(-1))[1][2] * Y), 
	                          (1 / Determinant(g^(-1))) * ((-1) * (g^(-1))[2][1] * X + (g^(-1))[1][1] * Y)>;
	            
	            if twist ge 0 then
	            	value := Pol ! (Determinant(g^(-1))^(twist) * emb(value));
				elif twist lt 0 then
					value := Pol ! ((1/(Determinant(g^(-1))^(Abs(twist)))) * emb(value));
				end if;
				
				for k := 1 to n do
					H_1[i][(j - 1) * n + k] := MonomialCoefficient(value, BasisFunctions[k]);
				end for;
			end for;
		end for;
...
function operator(R, type, N, P, irr, r, twist, graph_list)
	K := FieldOfFractions(R);
	Pol<X, Y> := PolynomialRing(K, 2);
	T := Name(K, 1);
	...
	if type eq 0 or type eq 1 then
		...
		BasisEdges, BasisFunctions, dimension := preparation(R, N, 1, irr, r, Graph, Reps);
		A := space(R, N, irr, r, twist, type, Graph, Reps);
		...
		for j := 1 to EdgeCount do
			...
			for g in SOR do
				...
				if counter eq 0 then
					...
					if type eq 0 or type eq 1 then
						add := cocycle(R, N, type, Graph, Reps, irr, r, twist, coeff, vec, vec * Matrix(K, 2, 2, [1, 0, 0, 1/T]));
					elif type eq 2 or type eq 3 then
						add := cocycle(R, N, 1, Graph, Reps, irr, r, twist, coeff, vec, vec * Matrix(K, 2, 2, [1, 0, 0, 1/T]));
					elif type eq 4 then
						add := cocycle(R, N, 0, Graph, Reps, irr, r, twist, coeff, vec, vec * Matrix(K, 2, 2, [1, 0, 0, 1/T]));
					end if;
					Append(~val, add);
					add := sign * add;
				else
				...
				emb := hom < Pol -> Pol | (1 / Determinant(delta)) * (delta[2][2] * X - delta[1][2] * Y), 
	                          (1 / Determinant(delta)) * ((-1) * delta[2][1] * X + delta[1][1] * Y)>;
	            
	            if twist ge 0 then
	            	add := Pol ! (Determinant(delta)^(twist) * emb(add));
				elif twist lt 0 then
					add := Pol ! ((1/(Determinant(delta)^(Abs(twist)))) * emb(add));
				end if;
				
				value := value + add;
			end for;
			
			for k := 1 to #BasisFunctions do
				T_P[i][k + (j - 1) * #BasisFunctions] := MonomialCoefficient(value, Pol ! BasisFunctions[k]);
			end for;
		end for;
	end for;
...
/*
Examples in Section 4.4 (before Section 4.4.1, no dual)

F := GF(3); R<T> := PolynomialRing(F); N := T; type := 1; P := T + 1; graph_list := [];
- for L(4, 1): irr := true; r := 3; twist := 1;
- for Sym^3 x det: irr := false; r := 3; twist := 1;
- for Sym^5: irr := true; r := 5; twist := 0;

A, graph_list := operator(R, type, N, P, irr, r, twist, graph_list);
*/
