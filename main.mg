...
function operation(R, dual, irr, r, twist, A, v)
	...
	if dual eq false then
		for i := 0 to n - 1 do
			for j := 0 to n - 1 do
				for k := 0 to i do
					for l := 0 to r - i do
						if k + l eq j then
							B[i + 1][j + 1] := B[i + 1][j + 1] + (-1)^(i - k + l) * Binomial(i, k) * Binomial(r - i, l) * a^(r - i - l) * b^(i - k) * c^l * d^k;
						end if;
					end for;
				end for;
				
				if twist ge 0 then
					B[i + 1][j + 1] := Determinant(A)^(twist) * (1/(Determinant(A)^r)) * B[i + 1][j + 1];
				elif twist lt 0 then
					B[i + 1][j + 1] := (1/(Determinant(A)^(Abs(twist)))) * (1/(Determinant(A)^r)) * B[i + 1][j + 1];
				end if;
			end for;
		end for;
	elif dual eq true then
		for i := 0 to n - 1 do
			for j := 0 to n - 1 do
				for k := 0 to j do
					for l := 0 to r - j do
						if k + l eq i then
							B[i + 1][j + 1] := B[i + 1][j + 1] + Binomial(j, k) * Binomial(r - j, l) * a^k * b^(j - k) * c^l * d^(r - j - l);
						end if;
					end for;
				end for;
				
				if twist ge 0 then
					B[i + 1][j + 1] := Determinant(A)^(twist) * B[i + 1][j + 1];
				elif twist lt 0 then
					B[i + 1][j + 1] := (1/(Determinant(A)^(Abs(twist)))) * B[i + 1][j + 1];
				end if;
			end for;
		end for;
	end if;
...
/*
 * Finally, the next function computes a Hecke operator T_P.
 *
...
function operator(R, type, N, P, dual, irr, r, twist)
	K := FieldOfFractions(R);
	T := Name(K, 1);
...
P := T + 1; Q := T^2 + 1; r := 3; twist := -r;
- for Gamma_1: type := 1;
- for Gamma_0: type := 3;

F := GF(3); R<T> := PolynomialRing(F); N := T; P := T + 1; dual := true; irr := true;
P := T + 1; Q := T^2 + 1; r := 4; twist := -4;
- for Gamma_1: type := 1;
- for Gamma_0: type := 3;

F := GF(3); R<T> := PolynomialRing(F); N := T; type := 1; P := T + 1;
